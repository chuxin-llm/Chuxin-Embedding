...
def get_args():
    ...
    parser.add_argument('--query_instruction_for_retrieval', default="为这个句子生成表示以用于检索相关文章：")
...
def find_knn_neg(model, input_file, candidate_pool, output_file, sample_range, negative_number, use_gpu):
    ...
    p_vecs = model.encode(corpus, batch_size=1024)
    print(f'inferencing embedding for queries (number={len(queries)})--------------')
    q_vecs = model.encode_queries(queries, batch_size=1024)
    ...
    for i, data in enumerate(train_data):
        ...
        data["pos"].extend([corpus[inx] for inx in pos_inxs])
